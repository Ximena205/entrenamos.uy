...
public class CInstitucion implements ICInstitucion {
	...
	public DtActividadDeportiva selectActividadDeportiva(String inst, String act) {
        ...
        for (String s : clases) {// hay que comentar porque al final no le paso la clase al DtActividadDeportiva
        	...
        	dtc.add(new DtClase(c1.getNombre(),c1.getFecha(),c1.getHoraInicio(),c1.getUrl(),c1.getFechaReg(), c1.getActividadDepo().getNombre()));
           i++;
       }
	...
	public boolean altaClase(String nombre_inst, String actividad, String nombre, DtFecha fechaIni, DtHora horaIni, String profesor, String url, DtFecha fechaAlta)throws ClaseRepetidaExcepcion {
		...
		//if(c != null) {
			...
			if(profe instanceof Profesor) {
				...
				List<Clase> claseprofe= ((Profesor) profe).getClases();
				String[] res1 = new String[claseprofe.size()];
				int i=0;
			        for(Clase cp:claseprofe) {
			        	res1[i]=cp.getNombre();
			        	if(res1[i].equals(nombre)) {
			        		throw new ClaseRepetidaExcepcion("La clase: [" + nombre + "] ya se dio de alta, intente nuevamente"); 
			        	}
			            i++;
			        }
	...
	*/
	...
	public DtClase selectClase(String inst, String act, String clase7) {
		...
		return new DtClase(c1.getNombre(),c1.getFecha(),c1.getHoraInicio(),c1.getUrl(),c1.getFechaReg(), c1.getActividadDepo().getNombre());
